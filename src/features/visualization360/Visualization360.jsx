...
import HouseVisualizerWithLighting from './components/HouseVisualizerWithLighting';
import ControlPanel from './components/UI/ControlPanel';
import ProductPanel from './components/UI/ProductPanel';
import { useVisualizationStore } from './store/visualizationStore';
import { Loader2, Eye, Box, Maximize2, Lightbulb } from 'lucide-react';
...
export default function Visualization360() {
  ...
  const [viewMode, setViewMode] = useState('lighting'); // 'lighting', '360' or '3d'

  return (
    <div className="visualization-360 h-full w-full relative bg-gray-900">
      {/* Main Viewer - Toggle between Lighting, 360, and 3D */}
      <div className="absolute inset-0">
        {viewMode === 'lighting' ? (
          <HouseVisualizerWithLighting />
        ) : viewMode === '360' ? (
          <House360Viewer />
        ) : (
      ...
      {/* Control Panel - Left Side (hidden in lighting mode) */}
      {ui.showControls && viewMode !== 'lighting' && (
        <div className="absolute top-4 left-4 w-80 max-h-[calc(100vh-120px)] overflow-y-auto z-10">
          <ControlPanel />
        </div>
      )}

      {/* Product Panel - Right Side (hidden in lighting mode) */}
      {ui.showControls && viewMode !== 'lighting' && (
      ...
      <div className="absolute top-4 left-1/2 transform -translate-x-1/2 z-20 flex items-center gap-2">
        ...
        {/* Toggle Controls Button (hidden in lighting mode) */}
        {viewMode !== 'lighting' && (
          <button
