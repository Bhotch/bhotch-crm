...
const FilterControls = ({ filterCriteria, setFilterCriteria }) => {import React, { useState, useEffect, useMemo } from 'react';
...
function App() {
  ...
  const handleAddLead = async (leadData) => {
    try {
      const response = await googleSheetsService.addLead(leadData);
      if (response.success && response.leadId) {
        await loadLeadsData(true); // Refresh data from server
        addNotification(`New lead added: ${leadData.customerName}`, 'success');
        setShowAddForm(false);
      } else {
        throw new Error(response.message || 'Failed to add lead in Google Sheets.');
        ...
        <div className="space-y-6">
            ...
            <div className="bg-white p-4 rounded-lg shadow">
                <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                    ...
                    <select value={filterDisposition} onChange={(e) => setFilterDisposition(e.target.value)} className="px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500"><option value="All">All Dispositions</option><option>New</option><option>Contacted</option><option>Qualified</option><option>Quoted</option><option>Follow Up</option><option>Closed Won</option><option>Closed Lost</option></select>
