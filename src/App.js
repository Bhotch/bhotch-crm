...
function doGet(e) {
  ...
  return createJsonResponse({ success: false, message: 'Invalid GET action.' }, 400);
}
...
function doPost(e) {
  try {
    ...
    switch (data.action) {
      ...
      default: return createJsonResponse({ success: false, message: 'Unknown action' }, 400);
    }
  } catch (error) {
...
function getLeads() {
  try {
    ...
    if (!sheet) return createJsonResponse({ success: false, message: `Sheet '${SHEET_NAME}' not found.` }, 404);
...
function updateLead(leadData) {
    // This function can be expanded with Geocoding similar to addLead if needed
    // For now, keeping it as is to avoid re-geocoding on every update.
    // A more advanced version could check if the address has changed.
    try {
        const sheet = getSheet();
        const data = sheet.getDataRange().getValues();
        const rowIndex = data.findIndex((row, i) => i > 0 && row[COLUMNS.id] === leadData.id);

        if (rowIndex === -1) return createJsonResponse({ success: false, message: 'Lead not found' }, 404);
        ...
        return createJsonResponse({ success: false, message: e.toString() }, 500);
    }
}


function deleteLead(leadId) {
  // Unchanged - simple and effective
  try {
    const sheet = getSheet();
    const data = sheet.getDataRange().getValues();
...
function geocodeAddress(address) {
    ...
    try {
        const geocoder = Maps.newGeocoder().setRegion('us'); // Bias results to the US
...
function createJsonResponse(data, statusCode = 200) {
    ...
    // This header is essential for allowing your web app to access the script
